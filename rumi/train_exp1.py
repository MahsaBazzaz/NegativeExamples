...
if opt.s > 0:
    epochs = opt.f - opt.s
    previous_d = f'./{opt.experiment}/{opt.game}/{opt.instance}/{opt.s}/RD*.pth'
    previous_g = f'./{opt.experiment}/{opt.game}/{opt.instance}/{opt.s}/RG*.pth'
...
for epoch in range(epochs + 1):
    ...
    # Process your positive and negative batches here
    # pos_batch contains a batch from dataloader_pos
    # neg_batch contains a batch from dataloader_neg
    ...
    if epoch % 100 == 0 and epoch > 0:
        ...
        torch.save(netG.state_dict(), f'{main_dir}/{epoch + opt.s}/RG_checkpoint_{epochs + opt.s}.pth')
        torch.save(netD.state_dict(), f'{main_dir}/{epoch + opt.s}/RD_checkpoint_{epochs + opt.s}.pth')
