...
if opt.game == "mario" or opt.game == "cave_treasures":
    if opt.cond == 1:
        ...
        y_pos = [[1, 0] for _ in range(len(X_pos))]
        ...
        y_pos = [[2, 0] for _ in range(len(X_pos))]
        ...
        y_pos = [[3, 0] for _ in range(len(X_pos))]
    ...
    y_neg_1 = [[1, 1] for _ in range(len(X_neg_1))]
    y_neg_2 = [[2, 1] for _ in range(len(X_neg_2))]
    y_neg_3 = [[3, 1] for _ in range(len(X_neg_3))]
    y_neg_0 = [[1, 1] for _ in range(len(X_neg_0))]
    y_neg_4 = [[1, 1] for _ in range(len(X_neg_4))]


    X_neg = np.concatenate((X_neg_0, X_neg_1, X_neg_2, X_neg_3, X_neg_4))
    y_neg = np.concatenate((y_neg_0, y_neg_1, y_neg_2, y_neg_3, y_neg_4))
...
if opt.s > 0:
    epochs = opt.f - opt.s
    previous_g = f"./{opt.experiment}/{opt.game}/{opt.instance}/{opt.s}/CondG*.pth"
    previous_d = f"./{opt.experiment}/{opt.game}/{opt.instance}/{opt.s}/CondD*.pth"
...
for epoch in range(epochs + 1):
    ...
    if epoch % 100 == 0 and epoch > 0:
        ...
        torch.save(netG.state_dict(), f'{main_dir}/{epoch + opt.s}/CondG_{opt.cond}_checkpoint_{epochs + opt.s}.pth')
        torch.save(netD.state_dict(), f'{main_dir}/{epoch + opt.s}/CondD_{opt.cond}_checkpoint_{epochs + opt.s}.pth')
    
make_sure_dir_exists(f"{main_dir}/{opt.f}")
torch.save(netG.state_dict(), f'{main_dir}/{opt.f}/CondG_{opt.cond}.pth')
torch.save(netD.state_dict(), f'{main_dir}/{opt.f}/CondD_{opt.cond}.pth')