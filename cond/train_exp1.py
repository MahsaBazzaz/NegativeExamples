...
if opt.game == "mario" or opt.game == "cave_treasures":
    ...
    y_pos_1 = [[1, 0] for _ in range(len(X_pos_1))]
    y_pos_2 = [[2, 0] for _ in range(len(X_pos_2))]
    y_pos_3 = [[3, 0] for _ in range(len(X_pos_3))]
...
for epoch in range(epochs + 1):
    ...
    if epoch % 100 == 0 and epoch > 0:
        ...
        torch.save(netG.state_dict(), f'{main_dir}/{epoch + opt.s}/CondG_checkpoint_{epochs + opt.s}.pth')
        torch.save(netD.state_dict(), f'{main_dir}/{epoch + opt.s}/CondD_checkpoint_{epochs + opt.s}.pth')
    
make_sure_dir_exists(f"{main_dir}/{opt.f}")
torch.save(netG.state_dict(), f'{main_dir}/{opt.f}/CondG.pth')
torch.save(netD.state_dict(), f'{main_dir}/{opt.f}/CondD.pth')