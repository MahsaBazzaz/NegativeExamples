...
if opt.game == "mario" or opt.game == "cave_treasures":
    if opt.cond == 1:
        X = get_positive_db(opt.game, 1)
    elif opt.cond == 2:
        X = get_positive_db(opt.game, 2)
    elif opt.cond == 3:
...
if opt.s > 0:
    ...
    epochs = opt.f - opt.s
    previous_d = f'./{opt.experiment}/{opt.game}/{opt.instance}/{opt.s}/VD*.pth'
    previous_g = f'./{opt.experiment}/{opt.game}/{opt.instance}/{opt.s}/VG*.pth'
...
for epoch in range(epochs + 1):
    ...
    if epoch % 500 == 0  and epoch > 0:
        ...
        torch.save(netG.state_dict(), f'{main_dir}/{epoch + opt.s}/VG_{opt.cond}_checkpoint_{epochs + opt.s}.pth')
        torch.save(netD.state_dict(), f'{main_dir}/{epoch + opt.s}/VD_{opt.cond}_checkpoint_{epochs + opt.s}.pth')

make_sure_dir_exists(f"{main_dir}/{opt.f}")
torch.save(netG.state_dict(), f'{main_dir}/{opt.f}/VG_{opt.cond}.pth')
torch.save(netD.state_dict(), f'{main_dir}/{opt.f}/VD_{opt.cond}.pth')